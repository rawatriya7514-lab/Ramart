// RAMART Enterprise Private Admin Engine
...
// Helper: Authenticated fetch wrapper with strict 401 handling
async function apiFetch(endpoint, options = {}) {
  ...
  try {
    ...
    if (res.status === 401) {
      ...
      document.cookie = "ramart_admin_token=; path=/; max-age=0";
    ...
    showWpToast("API error: " + err.message, "danger");
    return null;
  }
}
...
async function checkAuthAndInit() {
  ...
  if (check && check.authenticated) {
    document.getElementById("displayAdminUser").textContent = localStorage.getItem("ramart_admin_user") || "Administrator";
...
async function handleAdminLogout() {
  if (confirm("Are you sure you want to sign out from RAMART Administration?")) {
...
async function loadDashboardStats() {
  ...
  document.getElementById("breakdownProcessing").textContent = stats.processingOrders || 0;
  ...
  if (!stats.recentOrders || stats.recentOrders.length === 0) {
    recentEl.innerHTML = `
      <div style="text-align: center; padding: 2.5rem 1rem; color: var(--wp-text-muted);">
        ...
        <p style="font-size: 12px;">Real incoming customer orders will appear here automatically.</p>
      </div>
    `;
    return;
  }
      ...
      <tbody>
        ${stats.recentOrders.map(o => `
          <tr>
            ...
            <td><button class="wp-btn wp-btn-secondary wp-btn-sm" onclick="viewOrderInvoice('${o.id}')">Invoice</button></td>
          </tr>
        `).join("")}
      </tbody>
    </table>
...
async function handleProductFormSubmit(e) {
  ...
  if (res && res.success) {
    ...
    showWpToast(editId ? "Product updated successfully!" : "New product published!", "success");
  }
}
  ...
  tbody.innerHTML = filtered.map(o => `
    <tr>
      ...
      <td>
        ...
        <div style="font-size: 11px; color: #64748b;">${escapeHtml(o.email || '')}</div>
      </td>
      <td>
      ...
      <td>
        <span class="wp-pill wp-pill-new">${(o.items || []).length} item(s)</span>
        <strong style="display:block; margin-top: 4px;">${formatINR(o.total)}</strong>
      </td>
      <td>
        <div>${escapeHtml(o.paymentMethod)}</div>
        <span class="status-badge ${o.paymentStatus === 'Paid' ? 'status-delivered' : 'status-pending'}" style="font-size: 10px;">${o.paymentStatus}</span>
      </td>
      <td>
        ${o.courierPartner ? `<div style="font-size: 11px; font-weight: 700; color: #0284c7;">🚚 ${escapeHtml(o.courierPartner)}</div>` : ''}
        ${o.trackingNumber ? `<div style="font-size: 11px; color: #475569;">AWB: <strong>${escapeHtml(o.trackingNumber)}</strong></div>` : `<span style="font-size: 11px; color: #94a3b8;">Not Dispatched</span>`}
      </td>
      <td>
        <span class="status-badge status-${(o.orderStatus || 'pending').toLowerCase()}">${o.orderStatus}</span>
      </td>
      <td>
        <div class="table-actions">
          <button class="wp-btn wp-btn-primary wp-btn-sm" onclick="openDispatchModal('${o.id}')">Dispatch</button>
...
function openDispatchModal(orderId) {
  ...
  document.getElementById("dispatchOrderDisplay").value = `${order.id} - ${order.customerName} (₹${Math.round(order.total)})`;
  document.getElementById("dispatchStatus").value = order.orderStatus || "Pending";
  document.getElementById("dispatchCourier").value = order.courierPartner || "";
  document.getElementById("dispatchAwb").value = order.trackingNumber || "";
  document.getElementById("dispatchTrackingUrl").value = order.trackingUrl || "";
  document.getElementById("dispatchPaymentStatus").value = order.paymentStatus || "Pending";
...
async function handleDispatchFormSubmit(e) {
  ...
  const orderStatus = document.getElementById("dispatchStatus").value;
  const courierPartner = document.getElementById("dispatchCourier").value;
  const trackingNumber = document.getElementById("dispatchAwb").value.trim();
  const trackingUrl = document.getElementById("dispatchTrackingUrl").value.trim();
  const paymentStatus = document.getElementById("dispatchPaymentStatus").value;
  ...
  if (res && res.success) {
    ...
    await loadOrders();
    await loadDashboardStats();
    showWpToast(`Order #${orderId} dispatch details updated. Customer notified.`, "success");
  }
}
...
function viewOrderInvoice(orderId) {
  ...
  modalBody.innerHTML = `
    <div class="wp-invoice-box">
      ...
      ${order.courierPartner ? `<div class="wp-invoice-row" style="color:#0284c7;"><strong>Courier Partner:</strong> <span>${escapeHtml(order.courierPartner)}</span></div>` : ''}
      ${order.trackingNumber ? `<div class="wp-invoice-row" style="color:#0284c7;"><strong>AWB Tracking Number:</strong> <span>${escapeHtml(order.trackingNumber)}</span></div>` : ''}
...
async function loadCustomers() {
  ...
  if (currentCustomers.length === 0) {
    tbody.innerHTML = `<tr><td colspan="6" style="text-align:center; padding: 2rem; color: #64748b;">No registered customers yet. Customer records will be saved automatically when orders are placed.</td></tr>`;
    return;
  }
...
async function triggerTestEmail() {
  ...
  if (res && res.success) {
    showWpToast(`Test email sent successfully to ${targetEmail}!`, "success");
  } else {
